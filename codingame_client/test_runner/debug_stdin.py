"""python -m codingame_client.test_runner.debug_stdin INPUT_FILE TARGET_FILE [EXPECTED_OUTPUT_FILE] [--update-expected]
...
`launch.json` entry with `"module": "codingame_client.test_runner.debug_stdin"`--not run
standalone for its own sake. `codingame_client.test_runner.runner.run_solution_locally` is the
...
logic is `run_debug_stdin`, a plain function (not just a CLI entry point)--`codingame_client.
puzzle_manager.debug`/`codingame_client.contribution_manager.debug` are thin, package-aware
...
def run_debug_stdin(
    ...
    """The actual logic behind `python -m codingame_client.test_runner.debug_stdin`--see the
       module docstring for the full behavior. Callable directly (not just via CLI argv) so
       package-aware wrappers (`codingame_client.puzzle_manager.debug`/`codingame_client.
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
            prog="python -m codingame_client.test_runner.debug_stdin",
