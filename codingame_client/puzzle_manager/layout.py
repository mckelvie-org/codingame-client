"""Filename/directory-name constants for a puzzle working directory's on-disk layout--shared
   across `codingame_client.puzzle_manager` submodules. Deliberately not shared with
   `codingame_client.contribution_manager.layout`, even though a couple of names coincide (e.g.
...
"""The one real, editable/submittable file--never varies. Same rationale as
   ...
   inside `data/`), same as `codingame_client.contribution_manager`'s--disposable/regeneratable."""
...
"""Read-only reference copy of the puzzle's stub-generation script (see
   ...
   unlike `codingame_client.contribution_manager`'s Python-only trivial stub for *authoring* a new
