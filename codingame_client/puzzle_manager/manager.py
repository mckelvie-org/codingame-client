"""`CgPuzzleManager`: builds a puzzle working directory from an existing server-side puzzle
   ...
   Deliberately much simpler than `codingame_client.contribution_manager`: exactly one file is
   ...
   **Three-way state split (see `codingame_client.puzzle_manager.schema`/`.layout` for the exact
   ...
   - `.meta/` (`CgPuzzleServerData` + read-only `statement.html`/`stub_generator.cgstub`/`tests/`):
     ...
     `codingame_client.puzzle_manager.test_cases_dir`) holds each test case's downloaded
...
class CgPuzzleLocalTestResult:
    ...
    """Whether the run completed without crashing/timing out and its stdout matched the test
       case's expected output (see `codingame_client.test_runner.outputs_match`)."""
...
class CgPuzzleManager:
    """Builds/updates a puzzle working directory (`puzzle_dir`) against the server, via an
       ...
       simpler than `codingame_client.contribution_manager`) design this is backed by."""
    ...
    def tests_dir(self) -> Path:
        """Path to this working directory's `.meta/tests/` (downloaded test case input/output--see
           `codingame_client.puzzle_manager.test_cases_dir`)."""
    ...
    async def import_(
        ...
        """Build this working directory from an existing puzzle: resolves this codingamer's test
           ...
           `codingame_client.puzzle_manager.test_cases_dir`)--all read-only reference copies,
    ...
    async def push(self) -> int:
        """Submit the current local `data/solution.src` to the server for credit
           ...
           CAUTION: unlike `codingame_client.contribution_manager`'s `push()`, this always
    ...
    def play_local(
        ...
        """Run the current local `data/solution.src` against the downloaded `.meta/tests/` test
           ...
           `codingame_client.test_runner.run_solution_locally`) and comparing captured stdout to
           ...
           `codingame_client.test_runner.debug_stdin` (launched directly, not through this
        ...
        Args:
            ...
            timeout:    Per-test-case wall-clock timeout in seconds--see
                        `codingame_client.test_runner.DEFAULT_RUN_TIMEOUT_SECONDS`.
        ...
        Raises:
            ...
            CgLocalRunUnsupportedLanguageError: if `data/puzzle-data.json`'s `solution_language`
                                                 isn't yet supported by `codingame_client.
