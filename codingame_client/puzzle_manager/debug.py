"""python -m codingame_client.puzzle_manager.debug TARGET_FILE TEST_INDEX [--puzzle-dir DIR]

VS Code debugger launcher for a puzzle working directory's solution--a thin, puzzle_manager-aware
wrapper around `codingame_client.test_runner.debug_stdin.run_debug_stdin`. TARGET_FILE (VS Code's
own `${file}` macro) is used only to *infer* the puzzle working directory (see
`codingame_client.puzzle_manager.resolver.infer_puzzle_dir`) when `--puzzle-dir` isn't given--
TEST_INDEX then selects which downloaded `.meta/tests/` test case's input/output to run against
(see `codingame_client.puzzle_manager.test_cases_dir.list_downloaded_test_cases`).

No `--update-expected` here (unlike `codingame_client.contribution_manager.debug`)--`.meta/tests/`
is downloaded server truth, regenerated by `repair()`; there's nothing to accept a local run's
output as a new baseline for.

Not exported from `codingame_client.puzzle_manager`'s own `__init__.py`--like `test_runner.
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
            prog="python -m codingame_client.puzzle_manager.debug",
