"""Local working-directory management for solving existing CodinGame puzzles--much simpler than
   `codingame_client.contribution_manager` (which authors/edits a contribution): exactly one file
   (`data/solution.src`) is ever editable, so there is no git repository involved at all.

   See `CgPuzzleManager` for `import_`/`repair`/`diff`/`discard_local`/`push`/`play`/`play_local`--
   its module docstring covers the three-way state-split design in full;
   `codingame_client.puzzle_manager.schema` for the working directory's three manifest files
   (`CgPuzzleIdentity`/`CgPuzzleServerData`/`CgPuzzleData`);
   `codingame_client.test_runner` (a separate, package-agnostic package--not `puzzle_manager`'s
   own) for how `play_local`/VS Code debugging actually run a solution; and
   `codingame_client.puzzle_manager.resolver` for how a puzzle directory is located.
"""

from __future__ import annotations

from .manager import (
    DATA_SUBDIR_NAME,
    META_SUBDIR_NAME,
    SOLUTION_FILE_NAME,
    STATEMENT_FILE_NAME,
    STUB_GENERATOR_FILE_NAME,
    TESTS_SUBDIR_NAME,
    CgPuzzleDiscardResult,
    CgPuzzleLocalTestFailedError,
    CgPuzzleLocalTestResult,
    CgPuzzleManager,
    CgPuzzleManagerError,
)
from .resolver import (
    CG_PUZZLE_DIR_ENV_VAR,
    DEFAULT_PUZZLE_SUBDIR_NAME,
    CgPuzzleDirInferenceError,
    CgPuzzleDirNotFoundError,
    find_puzzle_dir,
    infer_puzzle_dir,
    resolve_puzzle_dir,
)
from .schema import (
    PUZZLE_IDENTITY_FILE_NAME,
    PUZZLE_SCHEMA_VERSION,
    CgPuzzleData,
    CgPuzzleIdentity,
    CgPuzzleServerData,
)

__all__ = [
    "CgPuzzleManager",
    "CgPuzzleManagerError",
    "CgPuzzleDiscardResult",
    "CgPuzzleLocalTestResult",
    "CgPuzzleLocalTestFailedError",
    "CgPuzzleIdentity",
    "CgPuzzleServerData",
    "CgPuzzleData",
    "PUZZLE_IDENTITY_FILE_NAME",
    "PUZZLE_SCHEMA_VERSION",
    "DATA_SUBDIR_NAME",
    "META_SUBDIR_NAME",
    "SOLUTION_FILE_NAME",
    "STATEMENT_FILE_NAME",
    "STUB_GENERATOR_FILE_NAME",
    "TESTS_SUBDIR_NAME",
    "CgPuzzleDirNotFoundError",
    "CgPuzzleDirInferenceError",
    "find_puzzle_dir",
    "resolve_puzzle_dir",
    "infer_puzzle_dir",
    "CG_PUZZLE_DIR_ENV_VAR",
    "DEFAULT_PUZZLE_SUBDIR_NAME",
]
