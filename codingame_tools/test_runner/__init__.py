"""Package-agnostic local test execution, shared by `codingame_tools.puzzle_manager` and
   `codingame_tools.contribution_manager`: subprocess-based batch execution
   (`runner.run_solution_locally`) and an in-process, debugger-launchable single-run entry point
   (`python -m codingame_tools.test_runner.debug_stdin`--see that module's docstring).
