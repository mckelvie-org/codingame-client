...
class CgFileUploadServletHelper(CgServletHelper["CgFileUploadServlet"]):
    """Helper methods for CgFileUploadServlet. Currently empty."""


class CgFileUploadServlet(CgServlet):
    ...
    def __init__(self, client: CgClient) -> None:
        ...
        self.helper = CgFileUploadServletHelper(self)
    ...
    async def __call__(
        ...
        """Upload a file to the CodinGame servers.
           ...
           Generates a multipart MIME POST request (via `CgRawClient.servlet_post`) to
           ...
           servers, which can be used to download the file later (see `CgFileServletServlet`)
