...
class CgFileServletServletHelper(CgServletHelper["CgFileServletServlet"]):
    """Helper methods for CgFileServletServlet. Currently empty."""


class CgFileServletServlet(CgServlet):
    ...
    def __init__(self, client: CgClient) -> None:
        ...
        self.helper = CgFileServletServletHelper(self)
    ...
    async def __call__(
        ...
        """Download a file from the CodinGame servers.
           ...
           Generates a GET request (via `CgRawClient.servlet_get_bytes`) to
        ...
        Args:
            id:        The globally unique ID of the file to download, as provided by the server
                       at upload time (see `CgFileUploadServlet`).
