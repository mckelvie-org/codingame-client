"""Downloads a puzzle's test case input/output files into the `.meta/tests/` subdirectory of a
   ...
   `codingame_tools.puzzle_manager.manager`'s module docstring for the `.meta/` trust model), so
   it's gitignored and always fully regenerated by `import_()`/`repair()`, never read back or
   diffed.

   Deliberately much simpler than `codingame_tools.contribution_manager.test_cases_dir`'s
   ...
   `codingame_tools.puzzle_manager.layout`'s module docstring for why the two packages are kept
   fully independent.
"""
